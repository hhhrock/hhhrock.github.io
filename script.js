...
if (board && scoreElement && highScoreElement && statusElement) {
  ...
  const placeFood = () => {
    ...
    } while (snake.some((segment) => isSameCell(segment, food)));
  };
  ...
  const render = () => {
    ...
    context.strokeStyle = 'rgba(73, 255, 138, .08)';
    for (let line = 0; line <= gridSize; line += 1) {
      context.beginPath();
      context.moveTo(line * cellSize, 0);
      context.lineTo(line * cellSize, board.height);
      context.stroke();
      context.beginPath();
      context.moveTo(0, line * cellSize);
      context.lineTo(board.width, line * cellSize);
      context.stroke();
    }
    drawCell(food, '#ffdf5d');
    snake.forEach((segment, index) => drawCell(segment, index === 0 ? '#d8fbdc' : '#49ff8a'));
  };
  ...
  const gameLoop = (timestamp) => {
    ...
    if (timestamp - lastStep >= stepDelay) {
      lastStep = timestamp;
      step();
    }
