...
if (board && scoreElement && highScoreElement && statusElement) {
  ...
  const render = (timestamp = performance.now()) => {
    ...
    if (flash) { if (timestamp >= flash.until) flash = null; else { const progress = Math.min((timestamp - flash.started) / (flash.until - flash.started), 1); const alpha = 1 - progress; const size = cellSize * (1.15 + progress * 2.4); const centerX = flash.cell.x * cellSize + cellSize / 2; const centerY = flash.cell.y * cellSize + cellSize / 2; context.globalAlpha = alpha; context.fillStyle = flash.colors[0]; context.fillRect(centerX - size / 2, centerY - size / 2, size, size); context.strokeStyle = flash.colors[1]; context.lineWidth = 3; context.strokeRect(centerX - size, centerY - size, size * 2, size * 2); context.strokeStyle = flash.colors[2]; context.strokeRect(centerX - size * 1.35, centerY - size * 1.35, size * 2.7, size * 2.7); context.globalAlpha = 1; } }
  };
  ...
  const step = () => { direction = nextDirection; const nextHead = { x: snake[0].x + direction.x, y: snake[0].y + direction.y }; const hitWall = nextHead.x < 0 || nextHead.x >= gridSize || nextHead.y < 0 || nextHead.y >= gridSize; if (hitWall || snake.some((segment) => sameCell(segment, nextHead))) return endGame(); snake.unshift(nextHead); if (sameCell(nextHead, food)) { score += 10; if (score > highScore) { highScore = score; localStorage.setItem('snake-high-score', String(highScore)); } const started = performance.now(); flash = { cell: { ...food }, colors: ['#fff59d', '#ff3cac', '#00e5ff'], started, until: started + 360 }; placeFood(); } else snake.pop(); updateStatus('진행 중'); render(); };
