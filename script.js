...
if (board && scoreElement && highScoreElement && statusElement) {
  ...
  const render = (timestamp = performance.now()) => {
    ...
    if (flash) { if (timestamp >= flash.until) flash = null; else { const progress = Math.min((timestamp - flash.started) / (flash.until - flash.started), 1); const alpha = 1 - progress; const size = cellSize * (1.15 + progress * 2.4); const centerX = flash.cell.x * cellSize + cellSize / 2; const centerY = flash.cell.y * cellSize + cellSize / 2; context.fillStyle = `rgba(255, 255, 190, ${alpha})`; context.fillRect(centerX - size / 2, centerY - size / 2, size, size); context.strokeStyle = `rgba(255, 255, 255, ${alpha})`; context.lineWidth = 3; context.strokeRect(centerX - size, centerY - size, size * 2, size * 2); context.strokeStyle = `rgba(73, 255, 138, ${alpha})`; context.strokeRect(centerX - size * 1.35, centerY - size * 1.35, size * 2.7, size * 2.7); } }
  };
